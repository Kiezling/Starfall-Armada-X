...
/**
 * Camera roll reference: the ship's own up axis, always. There is no world "up" in space, so
 * the camera never re-levels itself against one -- the player can pitch through the vertical
 * as many times as they like and the view simply follows the hull. Referencing the ship's up
 * is also never degenerate: it is perpendicular to the view direction by construction, so
 * `lookAt`'s cross product can't collapse the way a world-up reference does near the poles.
 */
...
/** Roll reference passed to `lookAt`: the ship's own up. See the note above. */
...
export class ChaseCamera {
  ...
  update(player: PlayerState, dt: number, rawDt: number, settings: Settings): void {
    ...
    // Roll reference: the ship's own up, so the camera rides the hull and never re-levels
    // itself against a world horizon that does not exist in space.
    if (this.deathCam) {
